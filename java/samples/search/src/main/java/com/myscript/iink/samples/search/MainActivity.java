...
public class MainActivity extends AppCompatActivity implements View.OnClickListener
{
  ...
  protected void onCreate(@Nullable Bundle savedInstanceState)
  {
    ...
    try
    {
      ...
      contentPart = contentPackage.createPart("Raw Content"); // Choose type of content (possible values are: "Text Document", "Text", "Diagram", "Math", "Drawing", and "Raw Content")

      // text recognition of 'Raw Content' part must be enabled for search feature
      if (contentPart.getType().equals("Raw Content"))
      {
        editor.getConfiguration().setBoolean("raw-content.recognition.shape", false);
        editor.getConfiguration().setBoolean("raw-content.recognition.text", true);
      }
    }
    catch (IOException e)
    {
    ...
    findViewById(R.id.button_do_search).setOnClickListener(this);
  ...
  {
    ((EditText)findViewById(R.id.edit_search_text)).getText().clear();
    searchView.clearSearchResult();
  }

  private void doSearch()
  {
    String searchWord = ((EditText)findViewById(R.id.edit_search_text)).getText().toString();
